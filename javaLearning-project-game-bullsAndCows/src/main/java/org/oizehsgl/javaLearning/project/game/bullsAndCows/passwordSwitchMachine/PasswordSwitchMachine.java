...
public class PasswordSwitchMachine {
    ...
    public GuessBo optimalGuess() {
        List<Integer> password = IntStream.rangeClosed(1, passwordBos.get(0).getPassword().size()).boxed().toList();
        List<GuessBo> guessBoList = Collections2.permutations(password).stream().map(GuessBo::valueOf).toList();
