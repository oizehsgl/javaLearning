...
public class PsmService {
  public static void main(String[] args) {
    ...
    List<List<Integer>> guessList =
        Collections2.permutations(demoPassword).stream().collect(Collectors.toList());
    List<List<Integer>> passwordList =
        Collections2.permutations(demoPassword).stream().collect(Collectors.toList());
    ...
    while (true) {
      ...
      Comparator<GuessResult> fourComparator =
          (o1, o2) ->
              ComparisonChain.start()
                  .compare(
                      o1.getFourProbability(),
                      o2.getFourProbability(),
                      Ordering.natural().nullsFirst())
                  .result();
      Comparator<GuessResult> guessResultComparator =
          (o1, o2) ->
              ComparisonChain.start()
                  .compare(
                      o1.getUnknownRightProbability(),
                      o2.getUnknownRightProbability(),
                      Ordering.natural().nullsFirst())
                  .result();
      Ordering<GuessResult> compound =
          Ordering.natural().nullsFirst().compound(fourComparator).compound(guessResultComparator);
