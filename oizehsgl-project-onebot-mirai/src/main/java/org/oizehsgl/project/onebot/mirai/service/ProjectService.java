package org.oizehsgl.project.onebot.mirai.service;
...
public class ProjectService {
  ...
  @Lazy @Resource @Getter private Map<String, ProjectConsumer> projectConsumerMap;

  public void start() {
    for (Map.Entry<String, ProjectConsumer> stringProjectConsumerEntry :
        projectConsumerMap.entrySet()) {
      ...
      Listener<GroupMessageEvent> listener =
          bot.getEventChannel().subscribeAlways(GroupMessageEvent.class, projectConsumer);
  ...
  public void toggle(ProjectEnum projectEnum, Boolean bool) {
    getProjectConsumer(projectEnum).ifPresent(p -> p.setProjectConsumerSwitch(bool));
  }
