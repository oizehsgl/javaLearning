package org.oizehsgl.javaLearning.library.guava.cache;
...
public class DemoCacheService {
    ...
    private final LoadingCache<Integer, String> cache = CacheBuilder.newBuilder()
            ...
            //.weakValues()
            //// 使用软引用存储值. 软引用只有在响应内存需要时,才按照全局最近最少使用的顺序回收. 考虑到使用软引用的性能影响,我们通常建议使用更有性能预测性的缓存大小限定(基于容量回收). 使用软引用值的缓存同样用==而不是equals比较值
            //.softValues()
            // endregion################
            // endregion########
            // region   ######## 移除监听
            ...
            .removalListener(notification -> {
                log.info("缓存移除[cause<{}>,key<{}>,value<{}>]", notification.getCause(), notification.getKey(), notification.getValue());
    ...
    /**
     * 存在返回缓存值
     * 不存在则调用Callable<? extends V> loader方法动态生成缓存值并存入缓存中
     *
     * @param key 键
     * @return {@link String}
