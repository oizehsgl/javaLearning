...
public class DemoStateMachineBuilder extends StateMachineBuilder {
    private final static String MACHINE_ID = "demoMachine";
    ...
    public StateMachine<DemoState, DemoEvent> build(BeanFactory beanFactory) throws Exception {
        StateMachineBuilder.Builder<DemoState, DemoEvent> builder = StateMachineBuilder.builder();
        // 配置
        ...
        builder.configureTransitions()
                //支付事件: 待支付->待发货
                .withExternal()
                .source(DemoState.S1).event(DemoEvent.E1)
                .action(demoStateMachineAction).target(DemoState.S2)
                .guard(demoStateMachineGuard)
                //发货事件: 待发货->待收货
                .and().withExternal().source(DemoState.S2)
                .event(DemoEvent.E2).action(demoStateMachineAction).target(DemoState.S3)
                //收货事件: 待收货->已完成
                .and().withExternal().source(DemoState.S3)
                .event(DemoEvent.E3).action(demoStateMachineAction).target(DemoState.S4);
